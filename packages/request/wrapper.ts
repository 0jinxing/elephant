...
const requestWrapper = (url: string, options: http.RequestOptions) => {
  ...
  if (agent && new RegExp(config.proxy.url_regex).test(url)) {
    ...
    try {
      agentResult =
        agent[(options.protocol || url.match(/(https?:)/)[1]).slice(0, -1)];
    } catch (error) {
      agentResult = null;
    }
...
function wrap(func: typeof http.request) {
  const origin = func;
  return function (
    url: Parameters<typeof http.request>[0],
    options?: http.RequestOptions,
  ) {
    if (typeof url === 'object') {
      if (url instanceof URL) {
        requestWrapper(url.toString(), options);
      } else {
        const req = url;
        requestWrapper(
          req.url ||
            req.href ||
            `${req.protocol}//${req.hostname || req.host}${req.path}`,
          req,
        );
      }
    } else {
